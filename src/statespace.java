...
class ServerUnit
{
    ...
    ServerUnit(OTUnit initState)
    {
        try {
            this.currentState = (OTUnit)initState.clone();
        }catch(Exception e)
        {
            e.printStackTrace();
        }
    ...
    Operation addLocalOperation(Operation op)
    {
        ...
        {
            assert(currentState!=lastState);
            Operation[] transOp=lastState.transform(lastState,op, globalList.toArray(new Operation[globalList.size()]));
    ...
    ArrayList<Operation> addGlobalOperation(Operation op)
    {
        ...
        if(globalList.size()==0)
        {
            assert(currentState==lastState);
            try {
                currentState = (OTUnit) lastState.clone();
            }catch (Exception e)
            {
               e.printStackTrace();
            }
...
class StateNode
{
   ...
   // boolean transform; // indicate whether the node has transformed yet
    ...
    StateNode(OTUnit otUnit)
    {
        this();
    //    transform=false;

        try {
            this.otunit = (OTUnit) otUnit.clone();
        }catch (Exception e)
        {
    ...
    static void transform(StateNode node)
    {
        ...
        Operation[] transOp=node.otunit.transform(node.otunit,node.localOp,node.globalOp);

        StateNode sn=new StateNode(node.NextGlobalState.otunit);
        sn.doOperation(transOp[0]);

        /*this is for debugging*/
        StateNode st=new StateNode(node.NextLocalState.otunit);
        st.doOperation(transOp[1]);
        assert(st.otunit.equals(sn.otunit));
...
class ClientUnit
{
    ...
    ClientUnit(OTUnit otunit)
    {
        initNode=new StateNode(otunit);
        lastNode=initNode;
        currentNode=initNode;
    }
    void addLocalOperation(Operation op)
    {
        StateNode sn=new StateNode(lastNode.otunit);
        sn.doOperation(op);
    ...
    void addGlobalOperation(Operation op,ArrayList<Operation>path)
    {
        ...
        p.setGlobalOp(op);
        StateNode q=new StateNode(p.otunit);
        q.doOperation(op);
