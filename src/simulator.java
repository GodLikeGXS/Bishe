...
class Simulator
{
        ...
        try {
            ...
            for (int i = 0; i < allPermutations.size(); i++)
            {
                ...
                for (int j = 0; j < permutation.size(); j++)
                {
                    ...
                    /*
                    System.out.println(el.get(event));
                    UFSUnit u=allUnit.get(executor);
                    if(u instanceof  Client)
                        Client.show((Client)u);
                    else
                        Server.show((Server)u);
                    System.out.println("\n\n");
                    if(j==7)
                ...
                if (checker())
                {
                    ...
                    pw.println(((Client) (allUnit.get(1))).unit.currentNode.otunit);
                }
                else
                {
                ...
                //        ((Client) (allUnit.get(j))).unit.currentNode.ufset.show();
                    for (int j = 1; j < allUnit.size(); j++)
                        pw.print(((Client) (allUnit.get(j))).unit.currentNode.otunit);
                }
                traceToOrigin();
            }
            pw.close();
    ...
    public static void main(String[] args)
    {
        ...
        CList init=new CList("HELLO");

        /*
        /UFSet init=new UFSet(5);
        init.init(new equClass[]{new equClass(new int[]{1,2,3}),
                new equClass(new int[]{4}),new equClass(new int[]{5})});
         */

        Client c1=new Client(init),c2=new Client(init),c3=new Client(init),c4=new Client(init);
        Server s=new Server(init);
        c1.register(s);
        c2.register(s);
        c3.register(s);
        c4.register(s);

        Operation o1=new InsertOp(1,1,'G');
        Operation o2=new InsertOp(3,2,'X');
        Operation o3=new InsertOp(2,3,'F');
        Operation o4=new DeleteOp(5,4);
        /*
        ...
        //Operation o5=new SplitOp(new int[]{3});
        */
